...
const eitherSequence = A.sequence(E.getApplicativeValidation(NonEmptyArray_1.getSemigroup()));
...
function factory(defaultConfig, components) {
    const inputValidation = (resource, input, config) => {
        ...
        return function_1.pipe(eitherSequence(validations), E.fold(function_2.identity, () => []), validations => E.right({ resource, validations }));
    };
    const mockOrForward = (resource, data, config, validations) => {
        ...
        const produceOutput = isProxyConfig(config)
            ? function_1.pipe(forwardCall(config)(components.logger.child({ name: 'PROXY' })), TE.orElse(error => {
        ...
        return function_1.pipe(produceOutput, TE.map(output => ({ output, resource, validations })));
    };
    return {
        request: (input, resources, c) => {
            const config = lodash_2.defaults(c, defaultConfig);
            return function_1.pipe(components.route({ resources, input }), E.fold(error => {
                if (!config.errors && isProxyConfig(config)) {
                    return function_1.pipe(components.forward({ data: input, validations: [] }, config.upstream.href, config.upstreamProxy)(components.logger.child({ name: 'PROXY' })), TE.map(createWarningOutput));
                }
                else
                    return TE.left(error);
            }, resource => function_1.pipe(TE.fromEither(inputValidation(resource, input, config)), TE.chain(({ resource, validations }) => mockOrForward(resource, input, config, validations)), TE.map(({ output, resource, validations: inputValidations }) => {
                const outputValidations = config.validateResponse
                    ? function_1.pipe(E.swap(components.validateOutput({ resource, element: output })), E.getOrElse(() => []))
                    : [];
                return {
                    output,
                    validations: {
