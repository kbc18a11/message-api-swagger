...
function findContentByMediaTypeOrFirst(specs, mediaType) {
    return function_1.pipe(specs, A.findFirst(spec => wildcardMediaTypeMatch_1.wildcardMediaTypeMatch(mediaType, spec.mediaType)), O.alt(() => A.head(specs)), O.map(content => ({ mediaType, content })));
}
