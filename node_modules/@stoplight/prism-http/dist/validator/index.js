...
const isMediaTypeSupportedInContents = (mediaType, contents) => function_1.pipe(O.fromNullable(mediaType), O.fold(() => true, mediaType => function_1.pipe(O.fromNullable(contents), O.fold(() => true, contents => !!contents.find(x => !x.mediaType || wildcardMediaTypeMatch_1.wildcardMediaTypeMatch(mediaType, x.mediaType))))));
...
const tryValidateInputBody = (requestBody, bundle, body, headers) => function_1.pipe(checkBodyIsProvided(requestBody, body), E.chain(() => {
...
const validateMediaType = (contents, mediaType) => function_1.pipe(O.fromNullable(mediaType), O.map(contentType.parse), O.chain(parsedMediaType => function_1.pipe(contents, A.findFirst(c => {
    ...
    return (wildcardMediaTypeMatch_1.wildcardMediaTypeMatch(parsedMediaType.type, parsedSelectedContentMediaType.type) &&
